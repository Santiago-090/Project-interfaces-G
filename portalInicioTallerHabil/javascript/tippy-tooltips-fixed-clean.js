...
document.addEventListener('DOMContentLoaded', function () {
  ...
  tippy.setDefaultProps({
    ...
    placement: 'auto',
    delay: [300, 100],
    maxWidth: null, // Eliminar restricción de ancho máximo
    popperOptions: {
      modifiers: [
        {
  ...
  const guideSteps = [
    ...
    },
    {
      ...
      content:
        'Haz clic en este botón para añadir productos a tu carrito. Cada producto seleccionado aparecerá en tu carrito de compras.',
      placement: 'top',
      step: 4,
      required: true
    },
  ...
  function showGuideStep(index) {
    ...
    try {
      ...
      if (index === 0 && elements.length === 0) {
        ...
        const fallbackSelectors = ['h1', '.container-BarraSuperior', 'header', 'body'];
  ...
  if (helpButton) {
    helpButton.addEventListener('click', function (e) {
      ...
      if (isVisible) {
        ...
        // Si no hay tooltips visibles, mostrar el primero
        const firstStep = document.querySelector('.guide-step-content');

        if (firstStep) {
          // Desplegar el primer tooltip manualmente
          const instance = firstStep.closest('.tippy-box');
          if (instance && instance._tippy) {
            instance._tippy.show();
          }
        }
      }
    });
...
// Función para forzar el cierre de todos los tooltips abiertos
...
// Ejemplo de uso: cerrar todos los tooltips al hacer clic en cualquier parte de la página
document.addEventListener('click', function (e) {
  // Cerrar tooltips si se hace clic fuera de ellos
  if (!e.target.closest('.tippy-box') && !e.target.closest('.btn-help')) {
    closeAllTooltips();
  }
});
